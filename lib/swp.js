...
class Swp {
    constructor(BITBOX, network = 'mainnet', grpcUrl=null, chronikUrl = null) {
        this.BITBOX = BITBOX;
    ...
    async uploadSignal(msgType, // exchange = 1,  escrow = 2
        ...
        let transactions = [];
        let txid = fundingUtxo?.outpoint?.txid;
        let satoshis = parseInt(fundingUtxo?.value);
        let vout = fundingUtxo?.outpoint?.outIdx;
        let stampsSatoshis = dataObj?.stampsSatoshis ?? 0;
        ...
        if(!Array.isArray(fundingUtxo)) {
            fundingUtxo = [{
                txid: fundingUtxo.outpoint.txid,
                satoshis: parseInt(fundingUtxo.value),
                vout: fundingUtxo.outpoint.outIdx
            }]
        }
    ...
    async uploadPayment(fundingUtxo,                    // object in form: { txid:'', satoshis:#, vout:# }
        ...
        for (let nId = 0; nId < chunkCount; nId++) {
            ...
            if (nId === 0) {
                ...
                satoshis = parseInt(transactions[nId - 1].outs[1].value);
            }
    ...
    async downloadTx(swapUri, decryptWif=null, progressCallback=null) {
        ...
        let tx = await this.chronikClient.tx(txid);
        let prevHash = tx.inputs[0].prevOut.txid;
        let metadata_opreturn_hex = tx.outputs[0].outputScript;
        ...
        for (let index = 0; index < downloadCount; index++) {
            ...
            // download prev txn
            let tx = await this.chronikClient.tx(prevHash);
            prevHash = tx.inputs[0].prevOut.txid;
            let op_return_hex = tx.outputs[0].outputScript;
    ...
    buildMetadataTx(config) {
        ...
        let stampsSatoshis = config?.stampsSatoshis ?? 0;
        let output = inputSatoshis + stampsSatoshis - metadataFee;
        ...
        for (let i=0; i < config.input_utxo.length; i++) {
            let paymentKeyPair = this.BITBOX.ECPair.fromWIF(config.input_utxo[i].wif);
            const sighashType = transactionBuilder.hashTypes.SIGHASH_ALL | transactionBuilder.hashTypes.SIGHASH_ANYONECANPAY;
            transactionBuilder.sign(i, paymentKeyPair, null, sighashType, config.input_utxo[i].satoshis);
        }
